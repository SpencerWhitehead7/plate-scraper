...
const createExtendedError = (name: string, statusCode: number): ExtendedErrorConstructor => class extends Error {
  statusCode: number;
...
export const incorrectCredsErr = new NotAuthenticatedErr(Error("incorrect credentials"));
export const notLoggedInErr = new NotAuthenticatedErr(Error("not logged in"));

const AlreadyAuthenticatedErr = createExtendedError("AlreadyAuthenticatedErr", 409);
export const alreadyLoggedInErr = new AlreadyAuthenticatedErr(Error("already logged in"));
